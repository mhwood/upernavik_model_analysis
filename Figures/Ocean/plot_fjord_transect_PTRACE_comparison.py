...
def read_model_transect_data(project_dir, fjord_name, experiment, year, month, var_name):
...
def plot_transects(project_dir, fjord_name, years, tracer_name,
    ...
    output_file = os.path.join(project_dir, 'Figures', 'Ocean','Fjord Transects',
                               fjord_name, fjord_name+' '+tracer_name+' Transect Comparison.png')
...
for fjord_name in ['Upernavik N','Kakivfaat','Ussing Braeer']:
    ...
    model_depth_control, model_distance_control, annual_model_grids_control = \
        read_model_transect_data(project_dir, fjord_name, 'baseline', 2020, 11, tracer_name)

    model_depth_melange, model_distance_melange, annual_model_grids_melange = \
        read_model_transect_data(project_dir, fjord_name, 'baseline_melange', 2020, 11, tracer_name)

    model_depth_iceplume, model_distance_iceplume, annual_model_grids_iceplume = \
        read_model_transect_data(project_dir, fjord_name, 'baseline_iceplume', 2020, 11, tracer_name)

    model_depth_melange_iceplume, model_distance_melange_iceplume, annual_model_grids_melange_iceplume = \
        read_model_transect_data(project_dir,fjord_name,  'baseline_melange_iceplume', 2020, 11, tracer_name)


    # plt.imshow(annual_model_grids_iceplume[2016])
    # plt.show()

    plot_transects(project_dir, fjord_name, years, tracer_name,
                   distance, surface, ice_polygon, bed_polygon,
                   model_depth_control, model_distance_control, annual_model_grids_control,
                   model_depth_melange, model_distance_melange, annual_model_grids_melange,
                   model_depth_iceplume, model_distance_iceplume, annual_model_grids_iceplume,
