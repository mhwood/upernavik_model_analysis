...
def compute_timeseries(results_dir, metric, var_name, year, month, depth_index, hFaC):

    if var_name in ['SIarea']:
        ...
        for chl_number in range(1,6):
            ...
            # profile = np.mean(grid, axis=(0,2, 3))
            # plt.plot(profile)
            # plt.show()
    ...
    for t in range(np.shape(grid)[0]):
        level_set = grid[t, :, :]
        if metric=='mean':
            timeseries[t] = np.mean(level_set[hFaC[0,:,:]!=0])
        elif metric=='median':
...
def write_timeseries_to_nc(project_dir, metric, experiment, var_name, dec_yrs, timeseries, depth):

    output_file = os.path.join(project_dir, 'Data', 'Models', 'Sea Ice',
                               var_name +'_'+experiment+ '_'+metric+'_timeseries.nc')
...
for experiment in experiments:
    ...
    write_timeseries_to_nc(project_dir, metric, experiment, var_name, dec_yrs, timeseries, depth)
